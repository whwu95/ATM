...
from engine_for_finetuning import train_one_epoch, validation_one_epoch, final_test, merge, charades_validation_one_epoch
...
class VideoCLIP(nn.Module):
    def __init__(self, clip_arch, num_classes = 174, embed_dim = 512, tsm = 'tokent1d', T= 8, dropout=0.0, emb_dropout=0.0, 
        ...
        ###!!!
        if clip_arch in ["EVA02-CLIP-L-14", "EVA02-CLIP-L-14-336", "EVA02-CLIP-bigE-14"]:
            from eva_clip import create_model_and_transforms
            
            weight_path={
                "EVA02-CLIP-L-14": "./clip_pretrain/EVA02_CLIP_L_psz14_s4B.pt",
                "EVA02-CLIP-L-14-336":"./clip_pretrain/EVA02_CLIP_L_336_psz14_s6B.pt",
                "EVA02-CLIP-bigE-14": "./clip_pretrain/EVA02_CLIP_E_psz14_s4B.pt"
                }
...
def get_args():
    parser = argparse.ArgumentParser('ATM training script for video classification', add_help=False)
...
def main(args, ds_init):
    ...
    ###!!!
    if 'k400-pretrain' in args.resume: 
        checkpoint = torch.load(args.resume, map_location='cpu')
        print("Auto resume pretrain: {}".format(args.resume))
        ...
        msg = model.load_state_dict(update_dict(checkpoint['module']), strict=False)
        del checkpoint
        print(msg)
    ...
    if args.layer_decay < 1.0:
        ...
        if 'EVA'in args.model:
            assigner = LayerDecayValueAssigner(list(args.layer_decay ** (num_layers + 1 - i) for i in range(num_layers + 2)),vit_arch = 'eva_clip')
        else:
            assigner = LayerDecayValueAssigner(list(args.layer_decay ** (num_layers + 1 - i) for i in range(num_layers + 2)),vit_arch = 'clip')
    else:
        assigner = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            if args.data_set == 'Charades':  #or args.data_set == 'anet':
                mAP = charades_validation_one_epoch(data_loader_val, model, device, dataset = args.data_set)
                print('mAP: ',mAP)
    ...
    ###!!! do multi-crop multi-clip test for video
    # preds_file = os.path.join(args.output_dir, str(global_rank) + '.txt')
    # test_stats = final_test(data_loader_test, model, device, preds_file)
    # torch.distributed.barrier()
    # if global_rank == 0:
